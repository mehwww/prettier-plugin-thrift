...
import {
  ...
  createMapFieldType,
  createParseError,
  creataePropertyAssignment as createPropertyAssignment,
  createStringLiteral,
  createTextLocation,
} from "./factory";
...
export function createParser(
  ...
  function parseInclude(): IncludeDefinition {
    ...
    const includeComments = getComments();
    consumeComments();
    const postComment = getPostComment(pathToken.loc.end.line);
    if (postComment) {
      includeComments.push(postComment);
    }
  ...
  function parseService(): ServiceDefinition {
    const serviceComments: Array<Comment> = getComments();
    ...
    consumeComments();
    serviceComments.push(...getComments(CommentPosition.InnerPost));
    ...
    consumeComments();
    const postComment = getPostComment(closeBrace.loc.end.line);
    if (postComment) {
      serviceComments.push(postComment);
    }
  ...
  function parseFunctions(): Array<FunctionDefinition> {
    ...
    while (!check(SyntaxType.RightBraceToken)) {
      if (check(SyntaxType.CommentBlock, SyntaxType.CommentLine)) {
        ...
        const postComment = getPostComment(func.loc.end.line);
        if (postComment) {
          func.comments.push(postComment);
        }
  ...
  function parseFunction(): FunctionDefinition {
    const functionComments: Array<Comment> = getComments();
  ...
  function parseConst(): ConstDefinition {
    const constComments: Array<Comment> = getComments();
    ...
    consumeComments();
    const postComment = getPostComment(initializer.loc.end.line);
    if (postComment) {
      constComments.push(postComment);
    }
  ...
  function parseTypedef(): TypedefDefinition {
    ...
    const typedefComments: Array<Comment> = getComments();
    consumeComments();
    const postComment = getPostComment(nameToken.loc.end.line);
    if (postComment) {
      typedefComments.push(postComment);
    }
  ...
  function parseEnum(): EnumDefinition {
    const enumComments: Array<Comment> = getComments();
    ...
    consumeComments();
    enumComments.push(...getComments(CommentPosition.InnerPost));
  ...
  function parseEnumMembers(): Array<EnumMember> {
    ...
    while (!check(SyntaxType.RightBraceToken)) {
      if (check(SyntaxType.CommentBlock, SyntaxType.CommentLine)) {
        ...
        readListSeparator();
        consumeComments();
        const postComment = getPostComment(member.loc.end.line);
        if (postComment) {
          member.comments.push(postComment);
        }
  ...
  function parseStructLikeInterface(): StructLike {
    const structComments: Array<Comment> = getComments();
    ...
    consumeComments();
    structComments.push(...getComments(CommentPosition.InnerPost));
  ...
  function parseFields(): Array<FieldDefinition> {
    ...
    while (!check(SyntaxType.RightBraceToken)) {
      if (check(SyntaxType.CommentBlock, SyntaxType.CommentLine)) {
        ...
        const nextToken = currentToken();
        const postComment = getPostComment(field.loc.end.line);
        if (postComment) {
          field.comments.push(postComment);
        }
        fields.push(field);
        if (isStatementBeginning(nextToken)) {
          throw reportError(
  ...
  function parseField(): FieldDefinition {
    ...
    const fieldComments: Array<Comment> = getSelfComments(endLoc.end.line);
  ...
  function readMapValues(): Array<PropertyAssignment> {
    ...
    while (true) {
      ...
      properties[properties.length - 1].comments.push(...getComments());

      if (check(SyntaxType.CommaToken)) {
        advance();
      }
      consumeComments();
      const postComment = getPostComment(value.loc.end.line);
      if (postComment) {
        properties[properties.length - 1].comments.push(postComment);
      }
  ...
  function getComments(pos?: CommentPosition): Array<Comment> {
    const current: Array<Comment> = comments;
    comments = [];
    return current.map((c) => ({ ...c, position: pos || c.position }));
  }

  function getPostComment(endLine: number): Comment | undefined {
    if (comments.length > 0) {
      if (comments[0].loc.start.line === endLine) {
        const comment = comments.shift() as Comment;
        return { ...comment, position: CommentPosition.Post };
      }
    }
  }
  ...
  function getSelfComments(line: number): Array<Comment> {
    ...
    while (comments.length > 0) {
      if (comments[0].loc.end.line <= line) {
        const position =
          comments[0].loc.start.line === line
            ? CommentPosition.Post
            : CommentPosition.Pre;
        selfComments.push({ ...comments.shift(), position } as Comment);
      } else {
        break;
      }
    }
